...
- **CLI** (`src/cli/`): `init`, `commit`, `context`, `annotate` commands via clap 4.
...
- **`--live` flag** on `ultragit annotate`: Reads `AnnotateInput` JSON from stdin, calls the MCP handler, prints `AnnotateResult` JSON to stdout.
...
- **Read pipeline** (`src/read/`, `1170c01`): `ReadQuery` + `retrieve_regions()` with file path normalization, anchor filtering, and line range overlap. CLI: `ultragit read <path> [--anchor] [--lines]`.
...
- **`list_annotated_commits`** added to `GitOps` trait: Lists all commits with ultragit notes.
- CLI: `ultragit deps`, `ultragit history`, `ultragit summary`.
- 27 new unit tests.
...
- `--squash-sources` and `--amend-source` flags on `ultragit annotate`.
- 24 new unit tests.

#### Feature 10: Team operations (`00b6a80`+`dbd437e`, merged `6a2d223`)

- **Notes sync** (`src/sync/`): `enable`, `status`, `pull` subcommands. Adds push/fetch refspecs for `refs/notes/ultragit` to git config.
- **JSONL export** (`src/export.rs`): Iterates all ultragit notes, serializes as `ExportEntry` (commit_sha, timestamp, annotation).
...
- CLI: `ultragit sync`, `ultragit export`, `ultragit import`, `ultragit doctor`.
...
- `resolve_author()`: Resolves correction author from `ULTRAGIT_SESSION` env var or git config.
- 10 new unit tests.

### Final state

- **52 source files**, ~10,500 lines of Rust
- **143 tests** across 7 test binaries, all passing
- **15 CLI commands**: init, commit, context, annotate, read, deps, history, summary, flag, correct, sync (enable/status/pull), export, import, doctor
- **13 modules**: cli, git, ast, schema, mcp, read, annotate, hooks, provider, agent, sync, config, plus doctor/export/import
- Annotations stored in `refs/notes/ultragit`, synced via standard git notes infrastructure
